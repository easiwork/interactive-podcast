...
export async function fetchAndParseRSS(feedUrl: string): Promise<FeedItem[]> {
  try {
    ...
    return feed.items.map((item) => {
      ...
      const isAudioItem = enclosure?.type?.startsWith('audio/') || 
      ...
      if (item['mediaThumbnail']) {
        ...
        thumbnailUrl = typeof thumbnail === 'string' 
          ? thumbnail 
      ...
      if (item['itemImage']) {
        ...
        if (!imageUrl) {
          imageUrl = typeof itemImg === 'string' 
            ? itemImg 
...
export async function fetchRSSFeedInfo(feedUrl: string): Promise<FeedInfo> {
  try {
    ...
    if (!feedImageUrl && feed['feedImage']) {
      ...
      feedImageUrl = typeof feedImg === 'string' 
        ? feedImg 
        : feedImg?.url || feedImg?.href;
    }
    ...
    throw new Error(`Failed to parse RSS feed info: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
} 