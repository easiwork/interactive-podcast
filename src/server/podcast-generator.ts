...
import { FeedItem, fetchAndParseRSS, fetchRSSFeedInfo } from "./rss-feed-parser";
...
`,
  ...
  if (!data.choices || data.choices.length === 0 || !data.choices[0].message || !data.choices[0].message.content) {
    logger.error("Invalid response structure from OpenAI for script generation:", data);
...
${JSON.stringify(articleData)}`,
  ...
  const data = await response.json() as OpenAIChatCompletionResponse;
  if (!data.choices || data.choices.length === 0 || !data.choices[0].message || !data.choices[0].message.content) {
    logger.error("Invalid response structure from OpenAI for notes generation:", data);
...
export async function generateFullPodcast(
  ...
  if (fs.existsSync(metadataPath) && !forceRegenerate) {
    logger.info(`Found existing podcast for feed ${rssFeedUrl} on ${today}, returning cached result`);
  ...
  const audioItemCount = sampleItems.filter(item => item.contentType === 'audio').length;
  ...
  logger.info(`Feed analysis: ${audioItemCount}/${sampleSize} items are audio. Determined feed type: ${isPodcastFeed ? 'PODCAST' : 'NEWS/TEXT'}. Will collect ${maxItems} item(s).`);

  // For podcast feeds, use direct playback instead of generating a new podcast
  if (isPodcastFeed) {
    logger.info(`Podcast feed detected. Using direct playback approach instead of podcast generation.`);
    const directPlayback = await getDirectPodcastForPlayback(rssFeedUrl, forceRegenerate);
    
    if (!directPlayback) {
      logger.warn(`No audio episodes found for direct playback in RSS feed ${rssFeedUrl}.`);
      ...
      await fs.promises.writeFile(emptyAudioFilePath, "No audio episodes found");
    ...
    logger.info(`Direct podcast playback setup completed for: ${directPlayback.title}`);
    return result;
  }
  ...
  if (feedItemsToProcess.length === 0) {
    ...
    if (!fs.existsSync(emptyAudioDir)) fs.mkdirSync(emptyAudioDir, {recursive: true});
    const emptyAudioFilePath = path.join(emptyAudioDir, "empty.mp3");

    try {
        const host1Voice = VOICE_MAP["Host 1"];
        const audioBuffer = await createAudioFromText("No content available.", host1Voice);
        ...
        logger.error("Failed to generate placeholder audio for empty podcast", ttsError);
  ...
  logger.info(`Fetched ${feedItemsToProcess.length} items from RSS feed. Processing them now.`);
  ...
  const articleNotesPromises = feedItemsToProcess.map(async (item) => {
    ...
    if (item.contentType === 'text') {
      if (!item.link && !item.content) {
        logger.warn(`Skipping item "${item.title || 'Untitled Text Item'}" due to missing link and content.`);
        return null;
      }
      try {
        let articleData: ArticleData | null = null;
        let sourceUrl = item.link || `text-item:${item.title?.replace(/\s/g, '_') || 'untitled'}`;
        let usedFallbackContent = false;

        if (item.link) {
          logger.info(`Attempting to extract content from URL: ${item.link} for item "${item.title}"`);
          try {
            articleData = await extract(item.link);
            if (!articleData?.content) {
                logger.warn(`Extraction from ${item.link} yielded no content. Will check RSS content.`);
                articleData = null; // Ensure we hit the fallback logic if content is empty
            }
          } catch (extractionError: any) {
            logger.warn(`Failed to extract content from URL ${item.link} (Error: ${extractionError.message}). Checking RSS content fallback.`);
        ...
        if (!articleData && item.content) {
          logger.info(`Using content directly from RSS feed for item "${item.title}" (length: ${item.content.length}).`);
          articleData = {
        ...
        if (usedFallbackContent) {
            logger.info(`Successfully used RSS content for notes generation for "${item.title}".`);
        } else {
            logger.info(`Successfully extracted article content for notes generation for "${item.title}".`);
        }
      ...
      logger.info(`Processing audio item: "${item.title || 'Untitled Audio Item'}". Attempting transcription.`);
      if (!item.audioUrl) {
        logger.warn(`Skipping audio item "${item.title || 'Untitled Audio Item'}" due to missing audioUrl.`);
        return null;
      }
      ...
      if (transcribedText === null) {
        logger.error(`Transcription failed for "${item.title || 'Untitled Audio Item'}" from ${item.audioUrl}. Skipping.`);
        return null;
      }
      if (transcribedText.trim() === "") {
        logger.warn(`Transcription resulted in empty text for "${item.title || 'Untitled Audio Item'}" from ${item.audioUrl}. Skipping.`);
        return null;
      }

      logger.info(`Successfully transcribed audio for "${item.title || 'Untitled Audio Item'}". Length: ${transcribedText.length} chars.`);
      ...
      logger.warn(`Unknown content type for item: ${item.title || 'Untitled Item'}`);
      return null;
    }
  });
  ...
  if (resolvedArticleNotes.length === 0) {
    ...
    const emptyScript = "No processable content found in the feed items for today's podcast.";
    ...
    try {
        const host1Voice = VOICE_MAP["Host 1"];
        const audioBuffer = await createAudioFromText("No processable content available.", host1Voice);
        await fs.promises.writeFile(emptyAudioFilePath, audioBuffer);
        logger.info(`Generated empty placeholder audio at ${emptyAudioFilePath}`);
    } catch(ttsError) {
        logger.error("Failed to generate placeholder audio for empty notes", ttsError);
  ...
  for (let i = 0; i < scriptLines.length; i++) {
    ...
    if (playOriginalAudioMatch && playOriginalAudioMatch[1]) {
      ...
      logger.info(`Encountered PLAY_ORIGINAL_AUDIO placeholder for URL: ${originalAudioUrl}`);
      let tempOriginalAudioPath: string | null = null;
      try {
        const response = await fetch(originalAudioUrl);
        if (!response.ok || !response.body) {
          throw new Error(`Failed to download original audio: ${response.statusText} from ${originalAudioUrl}`);
        }
        ...
        tempOriginalAudioPath = path.join(feedSpecificDir, `original_audio_${uuidv4()}${extension}`);
        
        logger.info(`Downloading original audio from ${originalAudioUrl} to ${tempOriginalAudioPath}`);
        ...
        logger.error(`Failed to download or save original audio from ${originalAudioUrl}:`, error);
        // Optionally, insert a short TTS saying "Error playing audio clip"
        try {
            const errorText = "We experienced an error trying to play an audio clip.";
            ...
            logger.warn(`Inserted placeholder TTS for failed original audio clip at segment ${i}`);
        } catch (ttsError) {
            logger.error(`Failed to generate placeholder TTS for failed original audio:`, ttsError);
      ...
      if (parts.length < 2) {
          logger.warn(`Skipping malformed script line (expected 'Host: Text'): "${line}"`);
          continue;
      }
      const host = parts[0];
    ...
    const voiceId = VOICE_MAP[host];
      ...
      if (!voiceId) {
          logger.warn(`Skipping line due to unknown host: "${host}" in line: "${line}"`);
          continue;
      }
      if (!text || text.trim() === "") {
          logger.warn(`Skipping line with empty text for host "${host}": "${line}"`);
          continue;
      }
    ...
    try {
      ...
      tempAudioFiles.push(filepath);
        ...
        logger.error(`Failed to generate TTS audio for line ${i} ("${line}"):`, error);
  ...
  if (tempAudioFiles.length === 0) {
    logger.warn("No audio segments were generated or downloaded. Creating an empty/placeholder podcast.");
    ...
    const emptyScriptForNoAudio = "An error occurred, and no audio content could be assembled for the podcast.";
    const emptyAudioFilePath = path.join(feedSpecificDir, "empty_final.mp3");
     try {
        const host1Voice = VOICE_MAP["Host 1"];
        const audioBuffer = await createAudioFromText("No audio content available.", host1Voice);
        await fs.promises.writeFile(emptyAudioFilePath, audioBuffer);
    } catch(ttsError) {
        logger.error("Failed to generate placeholder audio for no-audio-segments case", ttsError);
    ...
    await fs.promises.writeFile(metadataPath, JSON.stringify(result, null, 2));
    logger.info(`Podcast generation completed with placeholder audio due to no segments.`);
    return result;
  }
  ...
  logger.info(`Successfully combined all audio segments into ${finalCombinedAudioPath}`);
...
export async function getDirectPodcastAudio(
  ...
  if (fs.existsSync(transcriptionPath) && !forceRegenerate) {
    logger.info(`Found existing transcription for feed ${rssFeedUrl} on ${today}, returning cached result`);
    const existingResult = JSON.parse(fs.readFileSync(transcriptionPath, "utf-8"));
    return existingResult;
  }

  if (forceRegenerate && fs.existsSync(transcriptionPath)) {
    logger.info(`Force regenerate flag set. Overwriting existing transcription for feed ${rssFeedUrl} on ${today}.`);
  }

  // Fetch RSS feed and get the latest podcast episode
  const allFeedItems = await fetchAndParseRSS(rssFeedUrl);
  const latestPodcastEpisode = allFeedItems.find(item => item.contentType === 'audio');
  ...
  logger.info(`Found latest podcast episode: "${latestPodcastEpisode.title}" from ${latestPodcastEpisode.audioUrl}`);

  // Transcribe the audio using Whisper
  const transcribedText = await transcribeAudioFromUrl(latestPodcastEpisode.audioUrl);

  if (transcribedText === null || transcribedText.trim() === "") {
    logger.error(`Transcription failed or empty for episode "${latestPodcastEpisode.title}" from ${latestPodcastEpisode.audioUrl}`);
    return null;
  }

  logger.info(`Successfully transcribed podcast episode. Length: ${transcribedText.length} chars.`);
  ...
  // Save the result
  await fs.promises.writeFile(transcriptionPath, JSON.stringify(result, null, 2));
...
export async function getDirectPodcastForPlayback(
  ...
  logger.info(`Fetching direct podcast for playback from RSS feed: ${rssFeedUrl}`);
  ...
  if (fs.existsSync(playbackInfoPath) && !forceRegenerate) {
    logger.info(`Found existing playback info for feed ${rssFeedUrl} on ${today}, returning cached result`);
    const existingResult = JSON.parse(fs.readFileSync(playbackInfoPath, "utf-8"));
    return existingResult;
  }

  if (forceRegenerate && fs.existsSync(playbackInfoPath)) {
    logger.info(`Force regenerate flag set. Overwriting existing playback info for feed ${rssFeedUrl} on ${today}.`);
  }
  ...
  logger.info(`Found latest podcast episode for direct playback: "${latestPodcastEpisode.title}" from ${latestPodcastEpisode.audioUrl}`);
  ...
  let imageUrl = latestPodcastEpisode.imageUrl || latestPodcastEpisode.thumbnailUrl;
  if (!imageUrl) {
  ...
  // Save the result
  await fs.promises.writeFile(playbackInfoPath, JSON.stringify(result, null, 2));
